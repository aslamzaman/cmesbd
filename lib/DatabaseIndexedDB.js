...
const initializedData = () => {
    const keyObject = Object.keys(setting);
    const valueObject = Object.values(setting);
    keyObject.forEach(async (item, i) => {
        await set(item, valueObject[i]);
    })
...
export const getDataFromIndexedDB = async (key) => {
    try {
        ...
        if (Array.isArray(data)) {
            ...
            const init = initializedData();
            const keysArr = init.keyObject;
            const valuesArr = init.valueObject;
            const index = keysArr.findIndex(keyItem => keyItem === key);
            if (index !== -1) {
